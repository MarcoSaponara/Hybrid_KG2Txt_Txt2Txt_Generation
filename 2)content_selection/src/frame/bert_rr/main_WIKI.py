...
from bert_rr.data_pipe_WIKI_2 import (QSDataLoader, get_test_cc_ids, load_retrieved_sentences) ############################################################################
...
N_SUMMARY_SENTS = 10 
rel_scores_dp = path_parser.graph_rel_scores / rr_config.RELEVANCE_SCORE_DIR_NAME_WIKI
rank_dp = path_parser.summary_rank / rr_config.RR_MODEL_NAME_BERT_WIKI
#ir_rec_dp = path_parser.data / 'test_wiki_cluster'  # before: test_mn
ir_rec_dp = 'test'
...
def _dump(model, cluster_loader, dump_dp):
    ...
    dump_fp = join(dump_dp, str(cluster_loader.cid))
...
def rel_scores2rank():
    ...
    for cid in tqdm(cids):
        ...
        sentences = [[sent.replace('\n', '').strip() for sent in original_sents[0]]]
        rank_records = rank_sent.get_rank_records(sid_score_list, sents=sentences)
...
def tune():
    ...
    rr_tune_dp = join(path_parser.summary_rank, rr_config.RR_TUNE_DIR_NAME_BERT_WIKI)
    rr_tune_result_fp = join(path_parser.tune, rr_config.RR_TUNE_DIR_NAME_BERT_WIKI)
    ...
    for filter_var in tune_range:
        ...
        for cid in tqdm(cids):
            retrieval_params = {
                'model_name': rr_config.RR_MODEL_NAME_BERT_WIKI,
                'cid': cid,
...
def finer_tune():
    ...
    rr_tune_dp = path_parser.summary_rank / rr_config.RR_FINER_TUNE_DIR_NAME_BERT_WIKI
    rr_tune_result_fp = path_parser.tune / f'{rr_config.RR_FINER_TUNE_DIR_NAME_BERT_WIKI}.txt'
    with open(rr_tune_result_fp, mode='a', encoding='utf-8') as out_f:
        headline = 'Filter\tRecall\tF1\n'
        out_f.write(headline)
...
def rr_rank2records():
    rr_rec_dp = join(path_parser.summary_rank, rr_config.RR_RECORD_DIR_NAME_BERT_WIKI)
...
def select_e2e():
    ...
    select_sent.select_end2end_wiki(**params)
...
def compute_rouge():
    ...
    output = rouge.compute_rouge_for_wiki(text_dp)
    return output
