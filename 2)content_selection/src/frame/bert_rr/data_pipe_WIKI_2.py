...
#WIKI_PARSER = WikiParser(dataset_var=retrieve_dp)
...
def load_retrieved_sentences(retrieved_dp, cid):
    ...
    WIKI_PARSER = WikiParser(dataset_var=retrieved_dp)
    original_sents = [WIKI_PARSER.get_sentences(article=article) for article in WIKI_PARSER.get_articles(idx = cid)]
    original_sents = [original_sent for sublist in original_sents for original_sent in sublist]
...
def mask(rdf):
      ...
      return s + ' [MASK] ' + p + ' [MASK] ' + o
           
def mask_with_sub(masked_rdfs):
    with_sub = ''
    for idx, masked_rdf in enumerate(masked_rdfs):

        if idx == 0:
          with_sub += '[SUBQUERY] ' + masked_rdf
        else:  
          with_sub += masked_rdf
...
def get_query(retrieve_dp, cid): #n_summary_sents):
    #item = ['[MASK]', '.']
    #query = ' '.join(item * n_summary_sents)
    #return query
    WIKI_PARSER = WikiParser(dataset_var=retrieve_dp)
    query_rdf = WIKI_PARSER.get_query_rdf(idx = cid)
    
    masked_rdfs = [mask(rdf=rdf) for rdf in query_rdf]
    return mask_with_sub(masked_rdfs = masked_rdfs)
    


class ClusterDataset(Dataset):
    def __init__(self, cid, retrieve_dp, transform=None): # REMOVED : n_summary_sents, no_query, 
        ...
        self.query = get_query(retrieve_dp, cid) ############################################################
    ...
    def __getitem__(self, index):
        ...
        # build xx
        xx = build_bert_sentence_x(self.query, sentence=self.sentences[index], with_sub=True)
...
class ClusterDataLoader(DataLoader):
    def __init__(self, cid, retrieve_dp, transform=data_tools.ToTensor()): #  REMOVED : n_summary_sents, no_query,
