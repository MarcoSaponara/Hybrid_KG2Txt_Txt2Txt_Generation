...
class PathParser:
    def __init__(self, path_type):
        ...
        if path_type == 'local':
            self.proj_root = Path('/content/margesum') #GOOGLE COLAB
            #self.proj_root = Path('/homedtic/msaponara/margesum') #UPF CLUSTER
        elif path_type == 'afs':
        ...
        #print(f'content of proj_root :\n{os.listdir(self.proj_root)}')
        ...
        self.top_wiki = self.data / 'top_wiki'
        #####################################################################################
        ...
        #self.local_rouge_dir = '~/pyrouge/RELEASE-1.5.5/data ################################################## specify your ROUGE dir 
        
        self.local_rouge_dir = '/content/pyrouge/pyrouge/rouge/tools/ROUGE-1.5.5/data' ########### GOOGLE COLAB
        #self.local_rouge_dir = '/homedtic/msaponara/margesum/pyrouge/pyrouge/rouge/tools/ROUGE-1.5.5/data' ############# UPF CLUSTER
...
config_meta = yaml.safe_load(open(config_meta_fp, 'r', encoding='utf-8'))
...
config_model = yaml.safe_load(open(config_model_fp, 'r'))
...
rr_config_id = 'wiki'  # mn, cnndm
rr_iter = rr_config_id2iter[rr_config_id]
def load_bert_rr():
    if rr_config_id == 'wiki':
      root = path_parser.model_save / f'rr_{rr_config_id}'
    else:
